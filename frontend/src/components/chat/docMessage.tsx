...
const DocMessage = ({ own = false }: { own: boolean }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`w-full h-1/2 px-2 py-1 flex    text-start rounded-md  ${
            !own ? "bg-foreground-50" : "bg-white/20"
          }`}
        >
          <div className="h-[90%] pe-3 py-1 grid place-items-center relative">
            <FaFile
              size={"1.5rem"}
              className={`mt-1 ${
                !own ? "fill-violet-500" : "fill-white"
              } shadow-lg`}
            />
          </div>
          <div className="h-full flex-1 flex flex-col ">
            <p className={` opacity-75 text-sm`}>hello.apk</p>
        ...
        >
          <h1 className={`${!own ? "ml-[.5rem]" : "mr-[.5rem]"} `}>hi</h1>
          <div className="flex">
